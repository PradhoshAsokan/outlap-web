...
function SessionRow({ label, session, isGrandPrix }: { label: string, session?: Session, isGrandPrix?: boolean }) {
  ...
  return (
    <div className="flex justify-between items-center group/item">
      <span className={`text-[10px] md:text-[11px] uppercase tracking-widest font-bold ${isGrandPrix ? 'text-[#FF1801]' : 'text-white/40'}`}>{label}</span>
      <span className="text-[10px] md:text-xs font-mono text-white/80 group-hover/item:text-[#FF1801] transition-colors tabular-nums">
...
export default function CalendarPage() {
  ...
  return (
    <div className="p-4 md:p-8 min-h-screen bg-asphalt text-smoke-white font-mono flex flex-col items-center">
      <div className="w-full max-w-5xl">
        <h1 className="text-3xl md:text-4xl font-black text-f1-red uppercase italic mb-6 md:mb-8 tracking-tighter text-center lg:text-left">Grand Prix Calendar</h1>
        
        <div className="border border-white/5 bg-carbon rounded-xl p-4 md:p-8 mb-8 md:mb-12 text-center shadow-2xl relative overflow-hidden">
          ...
          <p className="text-2xl sm:text-3xl md:text-7xl font-black tracking-tighter text-smoke-white mb-2 md:mb-4 tabular-nums">
            {loading ? '00 : 00 : 00 : 00' : timeLeft || 'SEASON ENDED'}
          </p>
          <p className="text-[7px] md:text-xs uppercase tracking-[0.4em] text-f1-red font-black">
        ...
        {loading ? (
          ...
          <div className="space-y-3 md:space-y-4 pb-20">
            {races.map((race) => {
               ...
               return (
                <div key={race.round} className={`border border-white/5 rounded-lg overflow-hidden transition-all duration-300 ${isExpanded ? 'border-f1-red/50 bg-carbon shadow-2xl scale-[1.01]' : 'hover:border-white/20 bg-carbon/40'}`}>
                  <button 
                    onClick={() => toggleExpand(race.round)}
                    className={`w-full text-left p-3 md:p-6 flex justify-between items-center group ${isPast ? 'opacity-30' : 'opacity-100'}`}
                  >
                    <div className="flex items-center gap-3 md:gap-6 min-w-0 flex-1">
                      <span className="text-base md:text-2xl font-black text-silver/40 group-hover:text-f1-red transition-colors w-5 md:w-8">
                        {race.round.padStart(2, '0')}
                      </span>
                      <img 
                        src={`https://flagcdn.com/w40/${isoCode}.png`} 
                        alt={race.Circuit.Location.country}
                        className="w-5 md:w-8 h-auto shadow-lg border border-white/10 flex-shrink-0"
                      />
                      <div className="min-w-0">
                        <h2 className={`text-xs sm:text-sm md:text-2xl font-black italic uppercase transition-colors truncate ${isExpanded ? 'text-f1-red' : 'text-smoke-white group-hover:text-f1-red'}`}>
                          {race.raceName}
                        </h2>
                        <p className="text-[7px] md:text-[10px] uppercase tracking-widest text-silver/40 font-black truncate">
                          {race.Circuit.circuitName}
                        </p>
                      </div>
                    </div>
                    <div className="text-right flex-shrink-0 ml-2">
                      <p className="text-xs md:text-lg font-black text-smoke-white/80 tabular-nums leading-none">
                        {new Date(race.date).toLocaleDateString([], { month: 'short', day: 'numeric' })}
                      </p>
                      <p className="text-[7px] md:text-[10px] text-f1-red uppercase font-black mt-1">{isPast ? 'DONE' : 'NEXT'}</p>
                    </div>
                  </button>

                  {isExpanded && (
                    <div className="px-4 md:px-12 pb-6 md:pb-8 pt-1 md:pt-2 grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8 animate-in fade-in slide-in-from-top-2 duration-300">
                      <div className="space-y-3 md:space-y-4 lg:border-r border-white/5 lg:pr-8">
                      ...
                      <div className="flex flex-col items-center justify-center bg-asphalt/60 rounded-lg p-3 md:p-4 border border-white/5 group/map relative overflow-hidden h-32 md:h-40">
                         <CircuitMap circuitId={race.Circuit.circuitId} className="w-full h-full" color="white" />
                         <span className="text-[7px] md:text-[8px] font-black text-silver/20 uppercase italic tracking-widest mt-1 md:mt-2">
