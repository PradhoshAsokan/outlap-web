...
export default function PitWallPage() {
  ...
  if (!sessionActive) {
     return (
        <div className="p-4 md:p-8 bg-asphalt font-mono text-smoke-white min-h-screen">
           <div className="max-w-6xl mx-auto">
              <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 md:mb-12 border-b border-white/5 pb-6 md:pb-8 gap-4">
                 <div>
                    <h1 className="text-3xl md:text-6xl font-black text-f1-red uppercase italic tracking-tighter">Session Summary</h1>
                    <p className="text-xs md:text-xl text-silver/40 uppercase tracking-widest">{sessionName} — {meta.circuit_name}</p>
                 </div>
                 <div className="md:text-right">
                    <p className="text-[10px] text-silver/20 uppercase tracking-[0.3em] mb-1">Status</p>
                    <span className="px-4 py-1 bg-white/5 border border-white/5 rounded-full text-smoke-white font-bold text-[10px] md:text-xs uppercase">Off-Track</span>
                 </div>
              </header>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12">
                 ...
                 <div className="lg:col-span-2 space-y-4 md:space-y-6">
                    <h3 className="text-f1-red font-black italic uppercase tracking-widest text-[10px] md:text-sm underline decoration-2 underline-offset-8 mb-4 md:mb-8">Final Classification</h3>
                    <div className="bg-carbon border border-white/5 rounded-xl md:rounded-2xl overflow-hidden shadow-2xl">
                       <div className="overflow-x-auto">
                          <table className="w-full text-left">
                             ...
                             <tbody>
                                {results.map((r) => (
                                   <tr key={r.driver_number} className="border-t border-white/5 hover:bg-white/[0.02] transition-colors h-11 md:h-auto">
                                      <td className="p-2 md:p-4 text-base md:text-2xl font-black italic text-silver/20">{r.position}</td>
                                      <td className="p-2 md:p-4">
                                         <div className="flex items-center gap-3 md:gap-4">
                                            <div className="w-0.5 h-6 md:h-8 rounded-full shadow-glow" style={{ backgroundColor: TEAM_COLORS[r.driver_number] }}></div>
                                            <div>
                                               <p className="text-base md:text-xl font-black uppercase italic leading-none text-smoke-white tracking-tighter">
                                                  {DRIVER_CODES[r.driver_number] || 'DVR'}
                                               </p>
                                               <p className="text-[8px] md:text-[10px] text-silver/40 font-bold mt-0.5 md:mt-1 tracking-widest">
                                                  #{r.driver_number}
                                               </p>
                                            </div>
                                         </div>
                                      </td>
                                      <td className="p-2 md:p-4 text-[8px] md:text-[10px] font-bold uppercase">
                                         {r.dsq ? <span className="text-f1-red">DSQ</span> : 
                                          ...
                                          <span className="text-green-500/60 text-[7px] md:text-[8px]">Fin</span>}
                                      </td>
                                      <td className="p-2 md:p-4 text-right text-base md:text-2xl font-black text-f1-red tabular-nums">{r.points}</td>
                                   </tr>
                                ))}
                             </tbody>
                          </table>
                 ...
                 <div className="space-y-6 md:space-y-8">
                    <div className="bg-carbon border border-white/5 rounded-xl md:rounded-2xl p-4 md:p-6 shadow-2xl relative overflow-hidden group">
                       <h4 className="text-[8px] md:text-[10px] font-black text-f1-red uppercase tracking-widest mb-4 md:mb-6">Track Specs</h4>
  ...
  return (
    <div className="p-2 md:p-4 bg-asphalt font-mono text-smoke-white min-h-screen overflow-x-hidden">
      
      {/* LAYER 1: LIVE CONTROL */}
      <div className="lg:h-[calc(100vh-120px)] flex flex-col mb-6 md:mb-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-3 md:mb-4 px-2 gap-3 md:gap-4">
          <div className="flex items-center gap-3 md:gap-4">
            <h1 className="text-xl md:text-2xl font-black text-f1-red uppercase italic tracking-tighter text-glow">The Pit Wall</h1>
            <div className="h-4 w-[1px] bg-white/20"></div>
            <p className={`text-[8px] md:text-[9px] uppercase tracking-[0.2em] ${sessionActive ? 'text-green-500 animate-pulse' : 'text-silver/40'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 md:gap-4 flex-1 min-h-0">
          {/* Leaderboard */}
          <div className="lg:col-span-3 h-[350px] lg:h-full flex flex-col bg-carbon border border-white/5 rounded-lg overflow-hidden shadow-2xl">
            <div className="bg-steel px-2 md:px-3 py-1 md:py-1.5 text-[8px] md:text-[10px] font-bold uppercase tracking-widest text-f1-red">Timing / Gaps</div>
            <div className="flex-1 overflow-y-auto scrollbar-hide">
              <table className="w-full text-left text-[8px] md:text-[10px]">
                <thead className="text-silver/20 uppercase border-b border-white/5 sticky top-0 bg-asphalt/90 backdrop-blur-md">
                  <tr><th className="px-2 py-1 md:py-2">P</th><th className="px-2 py-1 md:py-2">Code</th><th className="px-2 py-1 md:py-2 text-right">Gap</th><th className="px-2 py-1 md:py-2 text-right">Int</th></tr>
                </thead>
                <tbody className="bg-black/20">
                  {leaderboard.map((d) => (
                    <tr key={d.driver_number} className="border-b border-white/[0.03] hover:bg-white/5 transition-colors h-6 md:h-7">
                      <td className="px-2 py-1 font-bold text-silver/40">{d.position}</td>
                      <td className="px-2 py-1 flex items-center gap-1 md:gap-1.5">
                        <div className="w-0.5 h-2 md:h-2.5 rounded-full shadow-[0_0_5px_rgba(255,255,255,0.2)]" style={{ backgroundColor: d.color }}></div>
                        <span className="font-black italic text-xs md:text-sm text-smoke-white/90">{d.code}</span>
                      </td>
                      <td className="px-2 py-1 text-f1-red tabular-nums font-bold tracking-tighter text-right">{d.gap}</td>
                      <td className="px-2 py-1 text-silver/30 tabular-nums text-[8px] md:text-[9px] text-right">{d.interval}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {/* Circuit Monitor */}
          <div className="lg:col-span-6 h-[300px] lg:h-full border border-white/5 bg-carbon/40 rounded-lg flex flex-col items-center justify-center relative overflow-hidden group shadow-inner">
             <div className="absolute top-2 md:top-3 left-4 text-[7px] md:text-[8px] font-black text-f1-red/40 uppercase tracking-widest z-10">Circuit Monitor</div>
             <div className="absolute top-8 md:top-12 left-4 flex flex-col gap-1.5 md:gap-2 z-10 hidden md:flex"><CircuitStat label="Track Length" value={meta.track_length_km + " km"} /><CircuitStat label="Total Laps" value={meta.number_of_laps.toString()} /><CircuitStat label="First GP" value={meta.first_grand_prix.toString()} /></div>
             <div className="absolute top-8 md:top-12 right-4 flex flex-col items-end gap-1.5 md:gap-2 z-10 text-right hidden md:flex"><CircuitStat label="Fastest Lap" value={meta.fastest_lap.time} /><CircuitStat label="Held By" value={meta.fastest_lap.driver} /><CircuitStat label="Year" value={meta.fastest_lap.year.toString()} /></div>
             <CircuitMap circuitId={circuitId} className="w-[85%] h-[85%]" showCars={true} />
             <div className="absolute bottom-3 md:bottom-4 left-1/2 -translate-x-1/2 text-center z-10 w-full px-4"><h2 className="text-lg md:text-xl font-black italic uppercase text-smoke-white tracking-tighter drop-shadow-lg">{meta.circuit_name}</h2><p className="text-[7px] md:text-[8px] text-silver/40 uppercase tracking-[0.4em] font-bold">{meta.location}</p></div>
          </div>

          {/* Race Control */}
          <div className="lg:col-span-3 h-[300px] lg:h-full flex flex-col bg-carbon border border-white/5 rounded-lg overflow-hidden shadow-2xl">
            <div className="bg-steel px-2 md:px-3 py-1 md:py-1.5 text-[8px] md:text-[10px] font-bold uppercase tracking-widest text-f1-red">Race Control</div>
            <div className="flex-1 overflow-y-auto p-2 md:p-3 space-y-1.5 md:space-y-2 text-[8px] md:text-[10px] scrollbar-thin scrollbar-thumb-white/10">
              {messages.map((m, i) => (
                <div key={i} className="border-l border-f1-red pl-2 md:pl-3 py-0.5 md:py-1 bg-white/[0.03] rounded-r-md"><p className="text-smoke-white/90 leading-relaxed">{m.message}</p></div>
              ))}
            </div>
          </div>
        </div>
      ...
      {/* LAYER 2: ANALYSIS */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6 mb-6 md:mb-8 pt-3 md:pt-4 border-t border-white/5">
        <div className="lg:col-span-7 bg-carbon border border-white/5 rounded-xl overflow-hidden shadow-2xl">
          <div className="bg-steel px-3 md:px-4 py-1.5 md:py-2 text-[8px] md:text-[10px] font-bold uppercase tracking-widest text-f1-red flex justify-between"><span>Tire Strategy & Pit Analysis</span><span className="text-silver/20 font-mono uppercase text-[7px] md:text-[8px]">Link_Active</span></div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 p-3 md:p-4">
             <div className="border border-white/5 rounded-lg overflow-hidden bg-asphalt/40">
               <table className="w-full text-left text-[8px] md:text-[10px]">
                 <thead className="bg-white/5 text-silver/40 uppercase font-bold"><tr><th className="p-1.5 md:p-2">Car</th><th className="p-1.5 md:p-2">Comp</th><th className="p-1.5 md:p-2">Age</th></tr></thead>
                 <tbody>{stints.map((s, i) => (<tr key={i} className="border-t border-white/5 hover:bg-white/[0.02]"><td className="p-1.5 md:p-2 font-black italic">#{s.driver_number}</td><td className="p-1.5 md:p-2 text-center"><span className={`px-2 md:px-3 py-0.5 rounded-sm font-bold text-[7px] md:text-[8px] ${s.compound === 'SOFT' ? 'bg-red-600 text-smoke-white' : s.compound === 'MEDIUM' ? 'bg-yellow-600 text-smoke-white' : 'bg-smoke-white text-black'}`}>{s.compound}</span></td><td className="p-1.5 md:p-2 text-silver/60 tabular-nums">{s.tyre_age} L</td></tr>))}</tbody>
               </table>
             </div>
             <div className="border border-white/5 rounded-lg overflow-hidden flex flex-col bg-asphalt/40">
                <div className="bg-white/5 p-1.5 md:p-2 text-[7px] md:text-[8px] font-bold uppercase text-silver/40 border-b border-white/5 text-center">Box Entry Timeline</div>
                <div className="flex-grow p-1.5 md:p-2 space-y-1.5 md:space-y-2 max-h-[140px] md:max-h-[160px] overflow-y-auto">{pits.map((p, i) => (<div key={i} className="flex justify-between items-center text-[8px] md:text-[10px] p-1.5 md:p-2 bg-white/5 border border-white/5 rounded-md group hover:border-f1-red/40 transition-all"><div className="flex items-center gap-1.5 md:gap-2"><div className="w-1 md:w-1.5 h-1 md:h-1.5 bg-f1-red rounded-full"></div><span className="font-black italic text-[10px] md:text-xs">#{p.driver_number}</span></div><div className="flex gap-3 md:gap-4 items-center"><span className="text-f1-red font-bold tabular-nums">{p.duration ? p.duration.toFixed(2) : '---'}s</span><span className="text-silver/20 text-[7px] md:text-[8px] font-bold uppercase">L{p.lap_number || '---'}</span></div></div>))}</div>
             </div>
          </div>
        </div>
      ...
      {/* LAYER 3: TELEMETRY */}
      <div className="border border-white/5 bg-carbon border border-white/5 rounded-xl p-4 md:p-6 shadow-2xl mb-12 md:mb-20">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 md:mb-6 px-2 gap-3 md:gap-4"><div className="flex items-center gap-2 md:gap-3"><h3 className="text-f1-red font-black italic uppercase text-xs md:text-sm tracking-widest">Telemetry grid</h3><div className="px-1.5 md:px-2 py-0.5 bg-green-500/20 text-green-500 text-[7px] md:text-[8px] font-bold rounded animate-pulse uppercase">Link_Active</div></div><span className="text-[7px] md:text-[8px] text-silver/20 uppercase font-mono tracking-widest">Global Hz: 2.7</span></div>
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2 md:gap-3">{Object.keys(DRIVER_CODES).map((num) => { const dNum = parseInt(num); const data = telemetry[dNum]; return (<div key={num} className="bg-asphalt/40 border border-white/5 p-2 md:p-3 rounded-lg hover:border-f1-red/50 transition-all group"><div className="flex justify-between items-center mb-2 md:mb-3"><span className="text-sm md:text-base font-black italic text-silver/40 group-hover:text-smoke-white transition-colors">{DRIVER_CODES[num]}</span><div className="w-0.5 md:w-1 h-2.5 md:h-3 rounded-full shadow-lg" style={{ backgroundColor: TEAM_COLORS[num] }}></div></div><div className="space-y-1.5 md:space-y-2"><TelemetryStat label="SPD" value={data?.speed.toString() || '---'} unit="KM/H" /><TelemetryStat label="RPM" value={data?.rpm.toString() || '----'} /><TelemetryStat label="GR" value={data?.n_gear.toString() || '-'} /><div className="h-0.5 md:h-1 w-full bg-white/5 rounded-full mt-1.5 md:mt-2 overflow-hidden shadow-inner"><div className="h-full bg-f1-red/60 transition-all duration-700" style={{ width: `${(data?.throttle || 0)}%` }}></div></div></div></div>); })}</div>
      </div>
    </div>
  );
}
...
function CircuitStat({ label, value }: { label: string, value: string }) {
  return (
    <div className="bg-asphalt/60 p-1.5 md:p-2 rounded border border-white/5 backdrop-blur-md min-w-[75px] md:min-w-[85px] shadow-lg">
       <p className="text-[6px] md:text-[7px] uppercase font-bold text-silver/30 tracking-widest mb-0.5">{label}</p>
       <p className="text-[8px] md:text-[10px] font-black text-f1-red uppercase tabular-nums">{value}</p>
    </div>
  );
}
...
function StatItem({ label, value, color = 'text-smoke-white' }: { label: string, value: string, color?: string }) {
  return (
    <div className="text-center px-2 md:px-4 border-r border-white/5 last:border-0">
      <p className="text-[7px] md:text-[8px] text-silver/30 uppercase font-bold mb-0.5 md:mb-1 tracking-widest">{label}</p>
      <p className={`text-[10px] md:text-sm font-black ${color}`}>{value}</p>
    </div>
  );
}
...
function TelemetryStat({ label, value, unit }: { label: string, value: string, unit?: string }) {
  return (
    <div className="flex justify-between items-baseline">
      <span className="text-[6px] md:text-[7px] text-silver/30 font-bold uppercase">{label}</span>
      <div className="flex gap-0.5 md:gap-1 items-baseline">
        <span className="text-[8px] md:text-[10px] font-black tabular-nums text-smoke-white">{value}</span>
        {unit && <span className="text-[6px] md:text-[7px] text-silver/20">{unit}</span>}
      </div>
    </div>
  );
}
